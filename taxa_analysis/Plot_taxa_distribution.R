...
# metadata file path
metadata_path <- "deepsea_meta.csv"
...
rdata_path <- "/Users/tjlam/Desktop/sujin_srb/taxa_analysis/deepsea_dada2_merged_workspace.RData"
#rdata_path <- NULL

## Dada2 Workflow ####
# Preprocessing, Generate Abundance, and Taxanomic Assignments
metadata = read.csv(metadata_path,row.names = 1)
mfnFs = read.delim(input_fasta_list_foward, sep= '\n',header = FALSE)
mfnRs = read.delim(input_fasta_list_reverse, sep = '\n',header = FALSE)

# Extract sample names, assuming filenames have format: SAMPLENAME_XXX.fastq
sample.names <- sapply(strsplit(basename(as.character(mfnFs[,1])), "_"), `[`, 1)

infiles = data.frame(samples = sample.names, forward=sort(mfnFs[,1]), reverse=sort(mfnRs[,1]))
...
if(preprocess == TRUE){
  ...
  fnFs = file.path(dataset_dir, mfnFs[,1])
  fnRs = file.path(dataset_dir, mfnRs[,1])
...
focus_taxa = "Family"
taxa.genus <- phyla_subset(taxa, meta, abundance.t, Consolidated_focus_abundnace, focus_taxa)
g.family <- sample_plot(taxa.genus , focus_taxa, proportion = TRUE, legend_rows = 5)
g.family <- g.family + facet_grid(~Location_simple, scales = "free", space = "free")

# Plot Genus
focus_taxa = "Genus"
taxa.genus <- phyla_subset(taxa, meta, abundance.t, Consolidated_focus_abundnace, focus_taxa)
g.genus <- sample_plot(taxa.genus , focus_taxa, proportion = TRUE, legend_rows = 5)
g.genus <- g.genus + facet_grid(~Location_simple, scales = "free", space = "free")


# Save Plots
ggsave(filename = 'genus_deepsea.pdf', plot = g.genus, width = 20, height = 8, units = "in")
ggsave(filename = 'order_deepsea.pdf', plot = g.order, width = 20, height = 8, units = "in")
ggsave(filename = 'family_deepsea.pdf', plot = g.family, width = 20, height = 8, units = "in")
